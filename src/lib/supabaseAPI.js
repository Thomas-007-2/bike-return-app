...
export const uploadPhoto = async (file, orderId, merchantId) => {
  try {
    // Android 14 fix: Validate file size and type before processing
    ...
    // Android 14 fix: Ensure proper file name
    const fileExt = 'jpg' // Always use jpg extension for compressed images
    const fileName = `${orderId}_${Date.now()}.${fileExt}`.replace(/[^a-zA-Z0-9.-]/g, '_')
    const filePath = `${merchantId}/${orderId}/${fileName}` 

    console.log('Uploading photo:', { orderId, merchantId, fileName });

    // Android 14 fix: Create blob with explicit type for better compatibility
    const blob = new Blob([file], { type: file.type || 'image/jpeg' })

    // Upload file to storage with Android 14 compatibility settings
    const {data: uploadData, error: uploadError} = await supabase.storage 
      .from('return-photos') 
      .upload(filePath, blob, {
        contentType: 'image/jpeg', // Set content type to JPEG
        cacheControl: '3600', // Android 14 fix: Add cache control
        upsert: false // Android 14 fix: Prevent overwrite issues
      }) 
...
export const callWebhook = async (orderId, storeId) => {
  try {
    ...
    // Android 14 fix: Add timeout and proper error handling
    ...
    const response = await fetch('https://cloud.activepieces.com/api/v1/webhooks/ZQWjCysxyz4YAUgjMQSeb', {
      ...
      headers: {
        ...
        'Accept': 'application/json', // Android 14 fix: Explicit accept header
      },
      body: JSON.stringify({
        order_id: orderId,
        store_id: storeId,
      ...
      // Android 14 fix: Add proper request settings
