    ...
    <div className="w-full max-w-4xl mx-auto p-4">
            ...
            <>
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm mb-4">
                  ...
                  <br />
                  Maximum file size: 2MB per photo | Target compression: 500KB
                  <br />
              ...
              >
                {photos.length >= maxPhotos ? 'Maximum Photos Reached' : 'Select Photos'}
              </button>
            </>
          )}
        </div>
      ...
      {process.env.NODE_ENV === 'development' && photos.length > 0 && (
        <div className="mt-6 bg-gray-50 rounded-lg p-4">
          <h5 className="font-semibold text-gray-700 mb-2">Debug Information</h5>
          <div className="text-sm text-gray-600 space-y-1">
            <p>Total photos: {photos.length}</p>
            <p>Total compressed size: {(photos.reduce((sum, photo) => sum + photo.compressedSize, 0) / 1024).toFixed(0)}KB</p>
            <p>Average compression: {photos.length > 0 ? (photos.reduce((sum, photo) => sum + parseFloat(photo.compressionRatio), 0) / photos.length).toFixed(1) : 0}%</p>
          </div>
        </div>
      )}
    </div>
